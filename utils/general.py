...
def build_targets(p, targets, model):
    """
        ...
        target是我们本来就有的那个目标。要用检测的目标和target来计算loss
        # 核心操作
...
# 也没有conf分支忽略阈值的操作
    ...
    for i in range(det.nl):  # 3种步长的feature map
        ...
        if nt:  # num_targets 该batch中的目标数量
            ...
            若gt_wh/anhor_wh 或 anhor_wh太大/gt_wh 超出hyp['anchor_t']，
            则说明当前target与所选anchor形状匹配度不高，该物体宽高过于极端，不应强制回归，将该处的labels信息删除，在该层预测中认为是背景
